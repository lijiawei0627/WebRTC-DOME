const { hSet, hGetAll, hDel } = require('./util/redis')
const { getMsg, getParams } = require('./util/common')
 ...
 * @param {Object} pub
 */
async function getUserDetailByUid(userId, roomId, userName, pub) {
	let res = JSON.stringify(({ "userId": userId, "roomId": roomId, "userName": userName, "pub": pub }))
	return res
}
...
async function onListener(s) {
	...
	let userName = getParams(url, 'userName')
	let pub = getParams(url, 'pub')
	console.log("client uid：" + userId + " roomId: " + roomId + " 【" + userName + "】online ")
	//user map
	userMap.set(userId, s)
	//room cache
	if (roomId) {
		await hSet(roomKey + roomId, userId, await getUserDetailByUid(userId, roomId, userName, pub))
		console.log("roomId", roomId)
		oneToRoomMany(roomId, getMsg('join', userId + ' join then room', 200, { userId: userId, userName: userName }))
	}
	...
	s.on('disconnect', () => {
		console.log("client uid：" + userId + " roomId: " + roomId + " 【" + userName + "】 offline ")
		userMap.delete(userId)
		if (roomId) {
			hDel(roomKey + roomId, userId)
			oneToRoomMany(roomId, getMsg('leave', userId + ' leave the room ', 200, { userId: userId, userName: userName }))
		}
	});
